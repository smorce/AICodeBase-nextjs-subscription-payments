...
- 【対応してみた】日本語翻訳するときに # が消されちゃっているのでプロンプトの調整が必要
  - やっぱりおかしくなっているので修正が必要。チャンクの分割は問題ないので翻訳プロンプトの方に原因がありそう。
  - そもそもロジックもイケていない。まずは日本語のドキュメントかどうか判断させて、日本語ドキュメントなら layout をそのまま使い、英語のドキュメントなら翻訳の方に回した方が良い。
  - 修正してみた → OK
...
- Marp用コンテンツと日本語用コンテンツからリファレンスを削除
